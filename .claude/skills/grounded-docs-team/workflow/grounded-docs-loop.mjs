...
export const meta = {
  ...
  description: '한 줄 문서요청 + 근거(repo 코드·구조·AGENTS.md·대화맥락) → 절차 4요소 뼈대(작동순서→지식근거→판단·채점기준→막힐 때)로 쓴 문서 1개(README/문서/폼답변)를 쓰는 verification-gated 루프. maker(문서 writer)→checker 4(근거·쉬움·구조/글자수·말투). 결정적 게이트(지어냄 0·전문용어 남발 0·4요소 순서·글자수 초과 0·말투 S1 0), keep-best, run ledger, 종료조건. 역할은 코드가 인라인 spawn.',
...
// 게이트는 전부 카운트(minimize/maximize, 실측). 느낌 합격 없음. success_conditions 5축(checker 4)과 1:1.
...
//   s1        = 말투(AI 티) S1 결정적 위반 수                                    == 0
//   tone      = 말투 자연스러움 점수(0~10, 사람 글=10)                           >= 8.0
...
const TONE_SCHEMA = {  // 말투(AI 티) 축 — s1_count(minimize, ==0) + tone_score(maximize, >=8). 정본 schemas/tone-verdict.schema.json.
  type: 'object', required: ['tone_score', 's1_count', 'verdict'], additionalProperties: true,
  properties: {
    tone_score: { type: 'number', minimum: 0, maximum: 10 },
    s1_count: { type: 'number', minimum: 0 },
    verdict: { type: 'string', enum: ['pass', 'revise'] },
...
// (E) 말투: 결정적 AI 티(S1) 후보 — ai-tell-taxonomy.md 의 이 저장소 반복 티를 코드로 선검사(보수적).
//     의문문 소제목(D) · 줄표(—) 제목 수사(C/J) · 과장 오프너(D). 심판 전 하한.
const TONE_S1_OPENERS = ['한눈에', '일목요연', '핵심만', '결론부터', '한마디로']
function toneHits(md) {
  const hits = []
  const body = text(md)
  for (const rawLine of body.split('\n')) {
    const h = rawLine.match(/^\s{0,3}#{1,6}\s+(.*\S)\s*$/)   // markdown 소제목
    if (!h) continue
    const title = h[1].trim()
    if (/[?？]$/.test(title)) hits.push(`tone:heading-question:${title.slice(0, 40)}`)   // 의문문 제목
    if (/[—–]/.test(title) || /\s-\s/.test(title)) hits.push(`tone:heading-dash:${title.slice(0, 40)}`) // 대시 수사 제목
  }
  const bodyLower = lower(body)
  for (const p of TONE_S1_OPENERS) { if (bodyLower.includes(lower(p))) hits.push(`tone:opener:${p}`) } // 과장 오프너
  return hits
}
...
말투 규칙(AI 티 금지): 사람이 쓴 글로 읽히게. 의문문 소제목("~인가요?") 금지(평서문 제목), 줄표(—)로 제목 꾸미기 금지, "한눈에"·"결론부터" 같은 과장 오프너 금지, 이모지·굵게·불릿 남용 금지, 같은 어미 반복(균일 리듬) 피해 문장 길이·어미를 섞어라.
...
const tonePrompt = (doc, detTone, round) =>
`너는 말투(AI 티) 심판(checker)이다. 문서가 사람이 쓴 글로 읽히는지, AI 티가 나는지 한 렌즈로만 본다. 워크스페이스 AI-tell 정본(ai-tell-taxonomy.md, 10대 카테고리 A~J, 심각도 S1 결정적/S2 강함/S3 약함) 기준. 재작성하지 마라 — 티 나는 구간만 지목·처방. 신선 채점, 후하게 금지. 내용·수치·구조 불변, 표현·리듬·서식만.
코드가 미리 잡은 S1 후보 ${detTone.length}건: ${JSON.stringify(detTone)} (의문문 소제목·대시 수사 제목·과장 오프너).
특히: 줄표(—)를 제목·수사 장치로(C/J) · 의문문 소제목("~인가요?")(D) · 과장 오프너("한눈에"·"결론부터")(D) · 이모지·굵게·불릿 등 시각장식 남용(J) · 같은 어미 반복·균일 리듬(E) · 병렬 상투("~뿐만 아니라 ~까지")(D/F).
[문서 종류] ${DOC_TYPE}
[문서 content] ${JSON.stringify(text(doc.content).slice(0, 8000))}
tone_score 0~10 = 말투 자연스러움(사람 글=10). s1_count = S1(결정적) 위반 수. 코드가 잡은 ${detTone.length}건은 최소 하한이다(그보다 적게 세지 마라).
verdict: s1_count==0 이고 tone_score>=8.0 이면 pass, 아니면 revise.
출력 JSON: {tone_score:<0~10>, s1_count:<정수>, verdict, offenders:[{cat,span,severity,fix}], fixes:[{where,problem,fix}]}  · round ${round}`
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  ...
  const detTone   = toneHits(cur.content)
  ...
  const [gv, pv, sv, tv] = await parallel([
    ...
    () => agent(tonePrompt(cur, detTone, round),                { label: `tone:R${round}`,      phase: 'Gate', schema: TONE_SCHEMA }),
  ])
  // 근거·글자수·말투S1 = max(코드 선검사, 심판) — 엄격 측(위반은 크게). 쉬움·구조·말투점수 = min(코드 하한, 심판) — 후한 심판 방어.
  ...
  const s1        = Math.max(detTone.length, tv ? (tv.s1_count || 0) : 0)                    // 말투 S1 위반 (==0), 코드 하한
  const tone      = Math.max(0, Math.min(10 - detTone.length, tv ? (typeof tv.tone_score === 'number' ? tv.tone_score : 0) : 0)) // 말투 (>=8), 코드가 잡은 만큼 눌림
  ...
  const gateTone      = s1 === 0 && tone >= 8.0
  const pass = gateGrounded && gatePlain && gateStructure && gateOverflow && gateTone

  // q: 위반 페널티 + 점수 보상 합성(높을수록 좋음). 느낌 아님 — 실측 파생.
  const q = -(grounded * 5) - (s1 * 5) - (overflowN * 0.05) + plain + structure + tone
  const verdict = pass ? 'pass' : 'revise'
  ledger.push({ round, grounded, plain: Number(plain.toFixed(2)), structure: Number(structure.toFixed(2)), tone: Number(tone.toFixed(2)), s1, overflow: overflowN, chars: cc, q: Number(q.toFixed(2)), verdict })
  log(`R${round}  지어냄 ${grounded} · 쉬움 ${plain.toFixed(1)}/10 · 구조 ${structure.toFixed(1)}/10 · 말투 ${tone.toFixed(1)}/10(S1 ${s1}) · 초과 ${overflowN}자 · ${verdict}`)

  if (q > best.q) best = { doc: cur, q }                     // keep-best
  if (pass) { log(`R${round} ✓ 전 게이트 통과(지어냄0·쉬움≥9·구조≥8·초과0·말투 S1 0·tone≥8) — 출하 준비`); break }
