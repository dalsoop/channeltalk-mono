...
| F-9 | 변경/출하를 `main` 에 직접 push·머지 | **branch → PR → merge** 로만(‌main 직접 push 금지, branch protection 으로 강제). 커밋·PR 규약 준수 [G] |
...
6. **git 출하 규율(F-9)** — 실측 통과분은 `main` 에 직접 push 하지 않는다. **feature branch → PR → merge**(main 은 branch protection 으로 직접 push 차단). 이 규율은 `git-flow` 에이전트가 루프의 마지막 단계로 수행한다(정본: 루트 `AGENTS.md` Git 워크플로).
