...
| F-9 | 변경/출하를 `main` 에 직접 push·머지, 또는 리뷰 없이(approve 0) 자기 코드 머지 | **Issue → PR → Review → Merge** 로만. `git-flow`(이슈·브랜치·커밋·PR·리뷰 위임) + **신선 `pr-reviewer`**(PR diff checker)가 루프의 마지막 단계. **머지 = 사람 게이트, approve ≥1 필수**(branch protection `required_approving_review_count:1` — 그냥 통과 없음). 에이전트는 자동 머지·approve 안 함. main 직접 push 금지 [G] |
...
6. **git 출하 규율(F-9) — Issue → PR → Review → Merge** — 실측 통과분은 `main` 에 직접 push 하지 않는다. 출하 스테이지는 **두 에이전트**가 루프의 마지막 단계로 수행한다(정본: 루트 `AGENTS.md` Git 워크플로):
   ...
   - **머지 = 사람 게이트, approve ≥1 필수** — branch protection 이 `required_approving_review_count:1` 을 강제하므로 **그냥 통과가 없다**. pr-reviewer `approve` verdict + 검증 green 이어도 에이전트는 머지·`gh pr review --approve` 를 하지 않는다 — **사람이 approving review 를 부여하고 머지**. `request_changes` 면 maker 로 되돌려 재수정→재리뷰.
   - **봇 작성으로 사람 approve 를 가능하게(정상 경로)** — solo repo 는 자기 PR self-approve 불가라, git-flow 가 PR 을 **GitHub App 봇**(`channeltalk-mono-bot[bot]`; `~/.env` 의 `GH_APP_*` → `tools/mint_bot_token.mjs` 설치토큰)으로 올린다 → 작성자≠사람이라 **dalsoop 이 approve → `--admin` 없이 머지**. 봇 크리덴셜 없으면 사람 작성 폴백 + admin override.
